...
12. [CSS ile Özelleştirme](#12-css-ile-özelleştirme)
13. [REST API Referansı](#13-rest-api-referansı)
14. [Lite ve Pro Karşılaştırması](#14-lite-ve-pro-karşılaştırması)
...
| **Konum sağlayıcı** | WooCommerce MaxMind, CloudFlare veya ipinfo.io |

**Nasıl Çalışır:**
1. Ziyaretçi siteye ilk geldiğinde IP adresi kontrol edilir
2. Ülke belirlenir ve eşleşen para birimi atanır
3. Ziyaretçi isterse farklı para birimi seçebilir (çerez ile kaydedilir)
...
## 14. Lite ve Pro Karşılaştırması
...
| Bayrak simgeleri (22+ para birimi) | Var | Var |
...
Üç yöntem:
1. **Tema widget alanı:** Temanızın header widget alanına bir "Kısa Kod" widget'ı ekleyin ve `[mhm_currency_switcher]` yazın
2. **Elementor:** Header template'inize Currency Switcher widget'ını sürükleyin
3. **PHP:** Temanızın `header.php` dosyasına `<?php echo do_shortcode('[mhm_currency_switcher]'); ?>` ekleyin
