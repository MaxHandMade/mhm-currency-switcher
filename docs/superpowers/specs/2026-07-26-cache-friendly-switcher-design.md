...
**Tarih:** 2026-07-26
**Revizyon:** v6 — Fable bağımsız spec-denetimi tur 1-5 sonrası (tur 5: GO-WITH-CHANGES) + 8 sahip kararı işlendi
**Durum:** PLAN-HAZIR — tur 5 mimariyi onayladı, kalan mekanik düzeltmeler işlendi
**Hedef sürüm:** v1.1.0
...
>
> **Tur 5 (GO-WITH-CHANGES):** mimariyi veya para yolunu bozan bulgu **yok**; 9-dallı tablo düşmanca yeniden okumada ayakta kaldı, `! empty` ve REQUEST_URI-tabanlı ilkel pinleri WC'nin gerçek davranışına karşı doğru çıktı, önceki 30+ bulgunun hiçbiri sessizce düşmemiş, ~40 satır-referansında drift yok. Kalanlar mekanikti: envanterin 16. üyesi + ölü anahtarların **aktivasyon-hook yazım yüzeyi** + readme'nin **ikinci** duyuru noktası + düz-permalink sınırı. Ayrıca envanter taramasının yan ürünü olarak **canlı bir para bug'ı** çıktı (§8.4 yüzde komisyon enum uyuşmazlığı) — ayrı ve öncelikli fix'e alındı.
>
> Bu revizyon hepsini kapatır. **Mimari iskelet beş turda da sağlam çıktı** — tur 3'ten beri düzeltmeler karar tablosunun sırası, bir dal koşulunun pinlenmesi ve envanter doğruluğuyla sınırlı kaldı.
...
- **Store API:** `WC()->is_rest_api_request()` **ve** route `/wc/store` ile başlıyor.
  ...
  **Düz kalıcı bağlantı eki (tur 5 / M-3):** `WooCommerce::is_rest_api_request()` yalnız REQUEST_URI'de `wp-json/` arar; düz permalink'te REST URL'leri `?rest_route=` biçimindedir → wc/v3 ve Store API karar 2/5'e değil karar 7'ye düşer. **Para yönü güvenli** (karar 7 "dönüştür" der, Store API yine doğru tahsil eder) ama §8.4'ün çifte-dönüşüm fix'i o sitelerde uygulanmaz. Bu yüzden ilkel `rest_route` query-var'ını da OR-dalı olarak kontrol eder — gerileme riski yok, fix'in kapsamı daralmaz.
...
2. **🔴 Ölü kontroller — gerçek envanter 14 üye.** Bu sınıfı **üç kez üst üste eksik saydım** (v3: 2 · v4: 4 · gerçek: 14). Sayım artık tahminle değil, üç kümenin kesişimiyle yapıldı: **(a)** sanitizer whitelist'i ([RestAPI.php:207-255](../../../src/Admin/RestAPI.php) — whitelist dışı her anahtar **sessizce düşer**), **(b)** React'ın yazdığı anahtarlar, **(c)** `src/` içinde okunan anahtarlar.
   ...
   | 15 | `currency.countries` ([RestAPI.php:548-552](../../../src/Admin/RestAPI.php)) | Sanitize ediliyor, **UI'sı yok** ([ManageCurrencies.jsx:69-81](../../../admin-app/src/components/tabs/ManageCurrencies.jsx) yalnız code/enabled/rate/fee/format yazar), tüketicisi yok (`CountryCurrencyMap` statik haritadır, bu alanı okumaz). #7'nin **aynı fonksiyondaki, altı satır aşağısındaki kardeşi** — tur 5 yakaladı | **Kaldır** |

   | 16 | Para birimi başına **yuvarlama** ayarı | Ters yönlü üye: motor okuyor ([Converter.php:82-92](../../../src/Core/Converter.php)), sanitizer kabul ediyor ([RestAPI.php:525-533](../../../src/Admin/RestAPI.php)), **UI'sı yok** → yalnız REST ile elle girilebilir; `readme.txt:26` "Fee & rounding configuration per currency" diye duyuruyor | **İmplemente et** (UI eklenir; sahip kararı) |

   **Tasfiyenin kapsaması gereken 2. yazım yüzeyi (tur 5):** ölü anahtarlar yalnız sanitizer'dan silinmekle bitmez — **aktivasyon hook'u** `provider`, `cache_duration`, `round_prices`'ı her temiz kurulumda yeniden tohumluyor ([mhm-currency-switcher.php:147-157](../../../mhm-currency-switcher.php)); oraya dokunulmazsa #1/#3/#4 sonsuza dek yeniden doğar. Aynı yerde bir de **şeması bozuk seed** var: `mhmcs_currencies` düz kod listesi olarak yazılıyor ([:134-144](../../../mhm-currency-switcher.php)) ama `CurrencyStore::load()` `{base_currency, currencies}` bekliyor ([CurrencyStore.php:74-79](../../../src/Core/CurrencyStore.php)) → seed **sessizce etkisiz**. Aynı turda düzeltilir. (`uninstall.php:24` `mhmcs_settings`'i komple sildiği için uninstall tarafı temiz.)
   ...
   **Kalıcı ders:** aynı sınıfı **dört turda dört farklı sayıyla** raporladım (2 → 4 → 14 → 15+). Mekanik kesişime geçmek 14'ü doğru saydırdı ama yetmedi: #15 kesişimin **dışında**, ikinci bir sanitize fonksiyonundaydı; ölü anahtarların **yazım yüzeyleri** (aktivasyon hook'u) hiç taranmamıştı; readme'nin **ikinci** duyuru noktası atlanmıştı. Ders iki katmanlı: (a) envanteri kesişimle çıkar, sezgiyle değil; (b) **kesişimin sınırlarını da denetle** — "hangi kümeyi eksik tanımladım?" sorusu, "kaç üye var?" sorusundan önce gelir. [[feedback_audit_the_class_not_the_report]] + [[feedback_measure_with_the_format_owner]] + [[feedback_gate_suppression_blindspot]] (kapıyı kurar kurmaz neyi göremediğini sına).
...
4. **🔴 Yüzde komisyon enum uyuşmazlığı — AYRI ve ÖNCE düzeltiliyor (sahip kararı), v1.1 kapsamı DIŞINDA.** Arayüz komisyon tipini `none`/`percent`/`fixed` yazıyor ([ManageCurrencies.jsx:334-348](../../../admin-app/src/components/tabs/ManageCurrencies.jsx)); sanitizer yalnız `fixed`/`percentage` kabul edip gerisini `fixed`'e zorluyor ([RestAPI.php:519-521](../../../src/Admin/RestAPI.php)); `Converter` `percentage` ise `kur × (1 + %/100)`, değilse **`kur + değer`** uyguluyor ([Converter.php:111-118](../../../src/Core/Converter.php)). Sonuç: "Yüzde %2" seçilince efektif kur `0.92 + 2 = 2.92` → fiyatlar ~3 katı, **ve efektif kur sepeti beslediği için yanlış tahsilat**. `percentage` değerini arayüz hiçbir zaman üretemediğinden yüzde komisyon **fiilen hiç çalışmamış**. Bu, #13'ün (`show_flag`/`show_flags`) **değer-enum kardeşi**. Para bug'ı olduğu için uzun v1.1 hattına bağlanmadı; ayrı ve öncelikli fix olarak çıkar. v1.1 planı bunu **tekrar ele almaz**, yalnız §9D kilidini miras alır.
5. **REST wc/v3 çifte dönüşümü.** `RestApiFilter::maybe_convert_product_response` ([RestApiFilter.php:112-119](../../../src/Integration/WooCommerce/RestApiFilter.php)) `$data['price']`'ı çeviriyor — o alan `$product->get_price()` çıktısı, yani `PriceFilter`'dan **zaten geçmiş**. Cookie + `?currency=` birlikteyken tutar **iki kez** dönüşür. Karar 2 düzeltir.
...
- **🔴 readme.txt düzeltmesi (§8.2) — İKİ yüzey:** var olmayan iki özellik hem **Key Features listesinde** (`readme.txt:34` "Payment gateway restrictions per currency", `:35` "Multilingual currency names") hem **sürüm notlarında** (`:93-94`) duyuruluyor. v5 yalnız sürüm notlarını kapsıyordu; tur 5 bunu yakaladı — yalnız orası düzeltilseydi WP.org paketi sahte özellikleri duyurmaya **devam edecekti**. İkisi birden düzeltilir. WP.org'a giden pakette yalnızca gerçekten yapılan iş duyurulur.
