...
class ClientSecretsTest extends TestCase {
	...
	public function test_returns_empty_strings_when_no_constants_or_env_set(): void {
		...
		$this->assertSame( '', ClientSecrets::get_feature_token_key() );
	...
	public function test_three_secrets_resolve_to_distinct_constants(): void {
		...
		putenv( 'MHM_CS_LICENSE_FEATURE_TOKEN_KEY=B' );
		putenv( 'MHM_CS_LICENSE_PING_SECRET=C' );

		$this->assertSame( 'A', ClientSecrets::get_response_hmac_secret() );
		$this->assertSame( 'B', ClientSecrets::get_feature_token_key() );
