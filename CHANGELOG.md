...
- **`mhmcs_fallback_rates_url`** — filters the fallback source URL, receiving the URL and the base currency code. Added because the shipped host is not reachable everywhere: from a Turkish network it resolves to a national block address and the request times out, while the primary API answers normally. Restoring the previous host is not an option (that is what the offloading rule forbids), so a store behind a block can now redirect the fallback to any endpoint serving the same payload shape instead of forking the plugin.
