...
- **The cache compatibility warnings no longer flap, and "Snooze until this changes" now holds.** Each front-end render recorded a two-valued verdict, so a render that *cannot* measure a problem counted as healthy: a customer opening the real cart page cleared the cart-constant warning and deleted every administrator's snooze, and the shop owner browsing their own store while logged in did the same to the mini-cart warning. The next page view brought the warning straight back. On a theme that breaks every page, each page with a different address also rewrote the warning (a database write per distinct URL) and changed the signature a snooze is pinned to. Now a render reaches one of three verdicts and an inconclusive one records nothing; the first page a problem is seen on stays its example while the problem lasts; only a clean render of that same page clears it (or a 404 at that page, if it was deleted); and saving cache compatibility off clears both warnings at once. The two notices now say exactly when they clear.
...
- **The fallback exchange-rate source is served from a different host.** Rates now fall back to `https://latest.currency-api.pages.dev/...` instead of the jsDelivr CDN. The payload and the upstream project are the same; only the host changed. WordPress.org keeps a fixed list of public CDN domains in its offloading check, and a shipped source naming one is an error regardless of what the URL actually fetches. A store on an allow-listed egress will need to permit the new host. See the new filter below if the host is unreachable from your network.

### Added

- **A second fallback rate source, so a blocked network is no longer a dead end.** The host serving the first fallback is unreachable from some national networks — Turkey, measured — and not merely by DNS: resolving it over DNS-over-HTTPS and connecting straight to the real Cloudflare addresses with the correct SNI still times out, while other Cloudflare hosts answer normally. A server there cannot route around it by changing resolvers, the previous host cannot be restored (it is on WordPress.org's offloading deny-list), and the upstream project documents no third mirror. So the chain now ends at [Frankfurter](https://frankfurter.dev/), a different provider serving European Central Bank reference rates: no API key, commercial use permitted, no attribution required. The trade is coverage — roughly thirty currencies rather than hundreds — which is the right trade for a source reached only when two others have already failed, and the set includes the currencies shops actually price in. Two alternatives measured better on coverage and were rejected: one requires a visible attribution link on the admin screen, the other publishes no terms of use at all.
- **`mhmcs_fallback_rates_url`** — filters a fallback source URL, receiving the URL, the base currency code and which source is being filtered (`currency-api` or `frankfurter`). The source is part of the signature because a store that has to move one host almost never wants to move the others.
...
- **Currencies with no minor unit, or three of them, are no longer given two decimals.** The fallback when a currency carried no explicit decimal setting was a hard-coded `2`, not the store's own configuration — so a shop adding JPY saw `¥100.00` and one adding BHD lost a digit of precision. The table is derived from ICU's minor-unit data; 37 of the 163 codes WooCommerce offers are not two-decimal currencies. The plugin does not depend on the `intl` extension for this.
- **The Advanced tab no longer claims nothing is scheduled right after scheduling something.** Switching a store from "Manual only" to a recurring interval and saving armed the event on the server, but the panel kept the schedule it had read when the page loaded and rendered "Automatic updates are switched on, but no update is scheduled. Re-save this setting to schedule one." Following that advice changed nothing, because a second save re-read nothing either; only reloading the page cleared it. The save response now carries the schedule it armed.
- **Saving unrelated settings no longer moves the next rate update.** The panel posts the whole settings object, and the scheduler asked whether the interval had been *submitted* rather than whether it had *changed* — so toggling anything on that screen tore the event down and re-armed it at the current moment, shifting a daily store's update time and making the next visit run a full rate sync. Saving now reconciles the schedule against the stored interval: it re-arms on a real change, restores an event that has gone missing while the setting still promises one, and clears one left standing behind "Manual only".
- **Rate-limited responses (HTTP 429) are no longer cacheable.** The public conversion endpoint sent no `Cache-Control` on that path, so a proxy or page cache could hold a rejection and serve it to callers who were not over the limit.
...
- **A write that never landed is no longer announced as a success, on any of the six paths that did it.** `CurrencyStore::save()` has always returned a bool and the callers discarded it. The first pass fixed the two the audit report named — `save_currencies()` and `sync_rates()` — which is exactly the mistake KANUN 0 describes: a reported defect is a sample, not an inventory. The independent pre-ZIP audit then found three more still standing, including the busiest one. The full set: the currency save, `save_settings()`, and all three sync paths (REST button, the hourly cron in `Plugin::run_scheduled_rate_sync()`, and `wp mhm-cs rates sync`), plus `LegacyOptionMigrator::run()`, which deleted the legacy rows and stamped itself done whether or not the copy had been written.

  Two structural changes keep the class closed rather than asking six call sites to remember. `RateProvider::commit_sync()` now owns the ordering — it stores the rates and only then stamps the sync, so a caller cannot reach the timestamp without going through the save. `Core\OptionWriter::write()` owns the "did it land" question, because `update_option()` returns false both for a failed write and for a value that already equalled the stored one; reading that flag directly turns every unchanged save into an error. Three source-derived gates (`PersistenceResultTest`) now fail the build if a new call site discards a store result, stamps a sync it does not own, or carries a migration payload with an unchecked write.

- **Upgrading from a version before 0.3.0 could lose the currency configuration.** `LegacyOptionMigrator::run()` copied the old options to their current names, then deleted the originals and recorded the migration as complete — without checking that the copy had been written. On an upgrade where that write failed there was nothing left and nothing would ever try again. It now returns before deleting anything, leaving the legacy rows in place for the next request.

- **A per-product fixed price could be stored in a form that reads back as zero.** Measured: `is_numeric('1e309')` is true, `floatval()` gives `INF`, `(string) INF` is the literal `"INF"`, and `(float) "INF"` is `0.0` — so the product was offered for nothing in that currency, ahead of any exchange rate, with every gate green because zero is a valid float. Negative fixed prices were accepted too and reached `woocommerce_product_get_price` as negative line totals. The product form and the variation rows held two copies of the same loop; both now go through one `ProductPricing::sanitize_fixed_price_map()`, which keeps the comma decimal separator European shops type.
